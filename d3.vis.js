...
(function() {
	...
	vis.bar = function(container, settings) {
		...
		var defaultSettings = {
		    ...
		    , palette5b: ["#00A0B0", "#EDC951", "#6A4A3C", "#CC333F", "#EB6841"]
		    , palette5c: ["#33D97C", "#F74414", "#E5BB36", "#C481C2", "#6F1F68"]
		    , palette5d: ["#CFF09E", "#A8DBA8", "#79BD9A", "#3B8686", "#0B486B"]
		    , palette5e: ["#3B8686", "#EBC1B0", "#EB9876", "#EB6F3B", "#EB4500"]
			, vertical: true
			, margin: {top: 100, left: 100, right: 100, bottom: 100}
			, width: 900
			, height: 500
			, barGap: .1
			, barStyle: "shape-rendering:crispEdges;"
			, xVar: 'x'
			, yVars0: ['y']
			...
			, xAxis: {
				...
				, orient: 'bottom'
				, scale: 'xScale0'
				, pathStyle: "shape-rendering:crispEdges;fill:none;stroke:#000;stroke-width:2;"
				, lineStyle: "fill:none;stroke:#000;stroke-width:2;"
				, labelSize: 14
				, labelWeight: 'bold'
				, title: false
				, titleAnchor: 'middle'
			...
			, yAxis1: {
				...
				, scale: 'yScale1'
				, pathStyle: "shape-rendering:crispEdges;fill:none;stroke:#000;stroke-width:2;"
				, lineStyle: "stroke:#000;stroke-width:2;"
				, labelSize: 14
				, labelWeight: 'bold'
				, title: false
				, titleAnchor: 'middle'
		...
		var s = settings || defaultSettings;
	 	...
	 	["xAxis", "yAxis0", "yAxis1"].forEach(function(axis) {
	 		...
	 		if (s[axis].labelStyle!==false) {
	 			...
	 			if (s[axis].labelAnchor) s[axis].labelStyle += " text-anchor:"+s[axis].labelAnchor+";"; 
		...
		bar.build = function() {
		    ...
		    ['xAxis', 'yAxis0', 'yAxis1'].forEach(function(axis) {
		    	if(s[axis].enabled===true) {
		    		...
		    		var thisAxis = self.g.append('g')
		      		...
		      		if (s[axis].display!==true) {
		      			thisAxis.style('display', 'none');
		      		}
			...
			if(s.legend) {
			    ...
			    (s.legend.boxStyle) 
			    	? legendBox.attr('style', function(d) { return s.legend.boxStyle+'fill:pathStyle'+self.colorScale(d); })
		...
		bar.draw = function(data) {
    		...
    		['xAxis', 'yAxis0', 'yAxis1'].forEach(function(axis) {
    			if(s[axis].enabled) {
    				var thisAxis = self.g.select('.axis.'+axis);
					if(s[axis].style) { //axis style
						var style = thisAxis.attr('style') || "";
						thisAxis.attr('style', style+s[axis].style);
					}
					if(s[axis].pathStyle) { //path style
						var selected = thisAxis.selectAll('path');
						var style = (selected.length!==0)
							? selected.attr('style')
							: "";
						thisAxis.selectAll('path').attr('style', style+s[axis].pathStyle);
					}
					if(s[axis].labelStyle) { //axis label style
						var selected = thisAxis.selectAll('g.tick text');
						var style = (selected.length!==0)
							? selected.attr('style')
							: "";
						thisAxis.selectAll('g.tick text').attr('style', style+s[axis].labelStyle);
					}
					if(s[axis].lineStyle) { //axis line style
						var selected = thisAxis.selectAll('g.tick line');
						var style = (selected.length!==0)
							? selected.attr('style')
							: "";
						thisAxis.selectAll('g.tick line').attr('style', style+s[axis].lineStyle);
					}
				}
    		})
